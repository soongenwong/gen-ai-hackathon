...
def load_dataframe(uploaded_file) -> pd.DataFrame:
    """
    Load the uploaded CSV into a pandas DataFrame and validate it's not empty.
    Throws clear ValueErrors for unsupported column types and malformed CSVs.
    """
    try:
        ...
        if df.empty:
            raise ValueError("Uploaded CSV file is empty - no data rows found.")
        ...
        if len(unnamed_cols) == len(df.columns):
            raise ValueError("CSV file appears to have no proper column headers - all columns are unnamed.")
        ...
        raise ValueError(f"Malformed CSV file - parsing failed: {str(e)}")
    
    except UnicodeDecodeError as e:
        raise ValueError(
            f"CSV file encoding issue - unable to read file: {str(e)}. "
        ...
        raise ValueError("Permission denied - unable to read the CSV file.")
